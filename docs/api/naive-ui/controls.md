...
4. **`resolveControlProps(field, surface)`** — 内部调用，合并 `field.ui.formControl` 基础值和 `field.ui.overrides[surface].formControl` 覆盖
...
const fields: NaiveCrudField[] = [
  ...
  },
  {
    ...
    type: 'money',
    ui: {
      formControl: { min: 0, step: 0.01, precision: 2 },
    },
  },
]
```
