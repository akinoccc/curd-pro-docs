...
| `setQuery` | `(partial: Partial<Query>) => void` | — | 来自 `useCrudList` 的 setQuery（必填） |
...
1. **初始化**：调用时立即从 `route.query[queryKey]` 读取并反序列化，写回 `setQuery`
2. **写入 URL**：监听 `query`（deep watch），防抖后调用 `router.replace({ query })`
3. **防循环**：从路由读取时设置内部标记 `isSyncingFromRoute`，避免写回时触发 watch 循环
4. **空值清理**：当 query 为空对象或所有值为 null/undefined/空字符串时，自动从 URL 删除该参数
